  ...
  return (
    <section className="mt-8 border-t pt-6">
      <div className="flex items-baseline justify-between mb-3">
        ...
        {s.updatedAt && <span className="text-[11px] text-muted-foreground/70">{L.asOf} {new Date(s.updatedAt).toLocaleDateString(L.dateLocale)}</span>}
      </div>
      ...
      {(s.rating || s.rationale) && (
        <div className="rounded-xl border border-border p-4 mb-5">
          ...
          {s.rationale && <p className="text-sm text-foreground leading-snug">{mdLinks(s.rationale)}</p>}
        </div>
      )}
      ...
      {s.googleTrends && (
        <div className="mb-5">
          ...
          <p className="text-sm text-foreground leading-snug">{mdLinks(s.googleTrends.trend_summary)}</p>
        </div>
      )}
