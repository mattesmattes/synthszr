...
interface BloomLanguageSwitcherClientProps {
  currentLocale: LanguageCode
  /** Serverseitig geladen (s. bloom-language-switcher.tsx) — kein Client-Fetch,
   *  also auch kein Ladezustand, der als Layout-Sprung sichtbar wird. */
  languages: ActiveLanguage[]
}

export function BloomLanguageSwitcherClient({
  currentLocale,
  languages,
}: BloomLanguageSwitcherClientProps) {
  ...
  useEffect(() => {
    ...
    // Defer one tick, damit der Container-Ref sicher hängt, bevor gescrollt
    // wird. Früher war das nötig, weil der Sprach-Fetch ein Re-Render auslöste;
    // die Liste kommt jetzt vom Server, das Deferral bleibt nur noch für das
    // Layout des smooth-scroll.
    const t = setTimeout(() => {
      setIsOpen(true)
  ...
  // Don't render language switcher if only one language, but still show companies link
    ...
    return (
      <div className="flex justify-center items-center gap-4 mb-6">
        ...
        {/* Zweite Fassung desselben Headers (nur eine Sprache aktiv). Muss
            ...
            auseinandergelaufen, deshalb hier dieselbe Wortmarke und dieselben
            Trennstriche wie unten. */}
