...
export async function runNewsletterFetch(options?: {
  ...
  try {
    console.log('[Newsletter Fetch] Starting...', options?.targetDate ? `for date ${options.targetDate}` : 'since last crawl')
    ...
    if (tokenError || !tokenData?.refresh_token) {
      ...
      return { success: false, newsletters: 0, articles: 0, emailNotes: 0, errors: 1, totalCharacters: 0 }
    }
    ...
    if (!sources || sources.length === 0) {
      ...
      return { success: true, newsletters: 0, articles: 0, emailNotes: 0, errors: 0, totalCharacters: 0 }
    }
    ...
    const fetchDate = options?.targetDate || todayDate
    ...
    const emails = options?.targetDate
    ...
    const articlesToProcess = articleUrls
      .filter(a => {
        if (seenArticleUrls.has(a.url)) return false
