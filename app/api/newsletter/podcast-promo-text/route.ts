...
export async function GET(req: NextRequest) {
  ...
  const titleLines = title ? Math.max(1, Math.ceil(title.length / 23)) : 0
  const subLines = subtitle ? Math.max(1, Math.ceil(subtitle.length / 33)) : 0
  const PAD_Y = 30
  const height =
    PAD_Y +
    40 + // headline line
    (titleLines ? 14 + titleLines * 60 : 0) +
    (subLines ? 8 + subLines * 50 : 0) +
    PAD_Y
  ...
  if (title) {
    children.push(
      React.createElement(
        ...
        {
          key: 't',
          style: { fontSize: 48, fontWeight: 700, color: '#000000', textAlign: 'center', marginTop: 14, lineHeight: 1.25 },
        },
        title
      )
    )
  ...
  if (subtitle) {
    children.push(
      React.createElement(
        ...
        {
          key: 's',
          style: { fontSize: 38, fontWeight: 400, color: '#000000', textAlign: 'center', marginTop: 8, lineHeight: 1.3 },
        },
        subtitle
      )
    )
